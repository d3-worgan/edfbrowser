...
int remez_lp_order(const double* fcuts, const double* mag __attribute__((unused)), const double* dev,
		double* weight) {
...
void build_grid(int lgfiltre, int nfcns, const double* bands, int nbands __attribute__((unused)), int lgrid, int nodd, const double* dev, const double* weight,
		double** FreqGrille, int* taillegrilledense, double** des, double** wt)
{
	int k, np = floor(0.5+(nfcns*bands[1]) / (0.5+(bands[1]-bands[2])));
	if (np==0)
		np++;
	int ns = nfcns+1-np;
	if (ns<=1)
