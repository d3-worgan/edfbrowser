    ...
    if(release_event->button()==Qt::LeftButton)
    {
        ...
        if (draw_zoom_rectangle) {
            ...
            if ((m_x > (mouse_press_coordinate_x + 50)) && (m_y > mouse_press_coordinate_y + 50)) {
                ...
                for (i = 0; i < mainwindow->files_open; i++) {
                    //std::cerr << "view times " << mainwindow->zoomhistory->viewtime[mainwindow->zoomhistory->pntr][i] << " = " << mainwindow->edfheaderlist[i]->viewtime << "\n";
                ...
                for (i = 0; i < signalcomps; i++) {
                    ...
                    //std::cerr << "voltspcm " << mainwindow->zoomhistory->voltpercm[mainwindow->zoomhistory->pntr][i] << " = " << signalcomp[i]->voltpercm << "\n";
                    mainwindow->zoomhistory->voltpercm[mainwindow->zoomhistory->pntr][i] = signalcomp[i]->voltpercm;

                    //std::cerr << "screen offset " << mainwindow->zoomhistory->screen_offset[mainwindow->zoomhistory->pntr][i] << " = " << signalcomp[i]->screen_offset << "\n";
                    mainwindow->zoomhistory->screen_offset[mainwindow->zoomhistory->pntr][i] = signalcomp[i]->screen_offset;

                    for (j = 0; j < signalcomp[i]->num_of_signals; j++) {
                        //std::cerr << "sensitivity " << mainwindow->zoomhistory->sensitivity[mainwindow->zoomhistory->pntr][i][j] << " = " << signalcomp[i]->sensitivity[j] << "\n";
                ...
                if (mainwindow->pagetime < 1)
                    mainwindow->pagetime = 1;
